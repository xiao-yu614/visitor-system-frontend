...
import type {
  VisitApplyItem,
  VisitApplyListParams,
  VisitStatus,
} from "@/types/visitApply";
...
const fetchData = async () => {
  loading.value = true;
  try {
    const result: any = await getApplyList(searchParams);

    console.log("申请列表接口返回 result:", result);

    /**
     * 兼容不同后端返回格式：
     * 1. result = { list, total }
     * 2. result = { records, total }
     * 3. result = { data: { records, total } }
     */
    ...
    console.log("最终表格数据 tableData:", tableData.value);
  } catch (error) {
...
const confirmApproval = async () => {
  ...
  try {
    ...
    ElMessage.success(
      currentApprovalType.value === "approve" ? "已通过申请" : "已拒绝申请",
    );
